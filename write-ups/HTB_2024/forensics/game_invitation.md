...
In this challenge we received word 2007 file.

Opening with LibreOffice shows one image of the event cyber apocalypse hacker royale and it also shows that this file has some macros attached.

![libreoffice image](/img/game_invitation_libre_office.png)

Taking a look at the macros:

![macros](/img/game_invitation_macros.png)

From my understanding of the code I found out that it opens itself, uses a xor function, create a file called `mailform.js`, executes the new file and then destroy the evidences.

So I got into a windows virtual machine to execute the code and remove the snippet that destroy the file `mailform.js`.

Then I got the content of `mailform.js`:
...
The value of `lVky(0)` is `vF8rdgMHKBrvCoCp0ulm` as we can see in the image below:

![macro argument](/img/game_invitation_mailform_arg.png)

Instead of using the function `eval()` we will print the value of `Iwlh` obtaining the flag:
