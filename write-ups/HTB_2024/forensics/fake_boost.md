...
Analyzing the `fake_boost.pcapng` I first found some HTTP requets. The first request is a request to `/freediscordnitro`:

![fake_boost.pcapng inside wireshark](/img/fake_boost_wireshark.png)

This request has the following content:
...
So probably it executed some malicious code. We can confirm it in the next image where there is a `POST` request:

![fake_boost.pcapng inside wireshark showing POST request](/img/fake_boost_wireshark_post.png)

I got the string `jozeq3n`, reversed it and decoded in `base64` obtaining a powershell script. Inside the powershell script was the first half of the flag in a variable called `part1`:
...
Then we find a function called `Create-AesManagedObject`, so probably this function was used to encrypt the user data.
...
The script first encrypted the data then send it to the attacker. So I got the data of the POST reqeust and put inside the variable `encryptedData` then decrypted it:
...
$decryptedData = Decrypt-String -key $AES_KEY -encryptedData $encryptedData
...
The email one has the sescond half of the flag in base 64.
